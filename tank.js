...
class Tank extends Base2DObj {
	...
	updateHitbox() { // This is not in constructor as Size reducing dropbox calls it
	...
	fire() {
		if (this.ammo) {
			var bullet = new this.bullet(this, this.cx + this.sizeX * Math.cos(this.angle) * 2 / 3, this.cy - this.sizeX * Math.sin(this.angle) * 2 / 3);
			canvases.bullets.items.push(bullet);
			this.ammo -= 1;
		}
	}

	set health(newHealth) {
		newHealth = (newHealth > 0) * newHealth;
