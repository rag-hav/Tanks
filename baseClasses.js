...
class Base2DObj {
	...
	collision(obj, firstCall = true) {
		if (!changeQueue.includes(this)) changeQueue.push(this);
		if (firstCall) {
			let u1x, u2x, u1y, u2y;
			u1x = this.velocityX;
			u1y = this.velocityY;
	...
	frame() {
		if (this.velocityX || this.velocityY || this.alwaysInQueue) {
			if (!changeQueue.includes(this)) changeQueue.push(this);
			this.move();
		}
	}
...
class Hitbox {
	...
	checkCollision(otherHitbox, firstCall = true) {
		if (this.currentCollisions.includes(otherHitbox) || otherHitbox.currentCollisions.includes(this)) {
			return false;
		}
		if (this.checkOverlap(otherHitbox, firstCall)) {
			this.currentCollisions.push(otherHitbox);
			if (!changeQueue.includes(this)) changeQueue.push(this);
			return true;
		}
		return false;
	...
	checkOverlap(otherHitbox, firstCall = true) {
		this.updateTrueCentre();
		if (!firstCall || (otherHitbox.radius + this.radius) > distanceBtwPoints(this.trueCentre, otherHitbox.trueCentre)) {
			...
			return firstCall && otherHitbox.checkOverlap(this, false);
		} else return false;
	}
	...
	frame() {
		this.currentCollisions = this.currentCollisions.filter((a) => a.checkOverlap(this));
